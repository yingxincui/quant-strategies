...
class MACrossHedge:
    ...
    def enable(self):
        """启用MA交叉对冲功能"""
        self.enabled = True
        logger.info("启用MA交叉对冲功能")
        
    def disable(self):
        """禁用MA交叉对冲功能"""
        self.enabled = False
        logger.info("禁用MA交叉对冲功能")
        ...
        try:
            ...
            if self.hedge_order:
                ...
                self.hedge_order.info = {
                    'reason': f"MA死叉开空 - 快线: {self.strategy.fast_ma[0]:.2f}, 慢线: {self.strategy.slow_ma[0]:.2f}",
                    'margin': margin,
                ...
                logger.info(f"MA死叉开空 - 合约: {self.hedge_contract_code}, 价格: {self.hedge_entry_price:.2f}, 数量: {hedge_size}手, "
                          f"止损价: {stop_loss:.2f}, 止盈价: {take_profit:.2f}")
                
        except Exception as e:
            logger.error(f"MA死叉开空失败: {str(e)}")
    ...
    def check_exit(self):
        ...
        if current_price >= stop_loss or current_price <= take_profit:
            ...
            logger.info(f"MA死叉对冲{reason} - 日期: {current_date}, 合约: {contract_code}, 当前价格: {current_price:.2f}, {reason}价: {take_profit if current_price <= take_profit else stop_loss:.2f}")
        ...
        if order.status in [order.Completed]:
            if order.isbuy():  # 买入豆粕期货（平空）
                ...
                order.info.update({
                    ...
                    'reason': f"MA死叉对冲平仓 - 合约: {contract_code}, 入场日期: {entry_date}, 入场价: {entry_price:.2f}, 平仓价: {trade_price:.2f}, 收益率: {return_pct:.2f}%"
                })
                
                logger.info(f"MA死叉对冲平仓 - 日期: {trade_date}, 合约: {contract_code}, 价格: {trade_price:.2f}, 盈利: {hedge_profit:.2f}, "
            ...
            self.hedge_order = None
            logger.warning(f'MA死叉对冲订单失败 - 状态: {order.getstatusname()}')
